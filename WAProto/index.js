...
function createProtoClass(typeName) {
  return {
    encode(obj) {
      return {
        finish() {
          // Bridge handles camelCase→snake_case conversion and float truncation in Rust
    ...
    decode(buffer) {
      ...
      // Bridge CamelSerializer handles: camelCase keys, Uint8Array bytes, skip defaults
...
// Nested types used with .create() — passthrough
...
export const proto = {
  ...
  HistorySync, KeepType, LIDMigrationMappingSyncPayload, MediaRetryNotification, MediaVisibility,
  Message, PinInChat, SenderKeyDistributionMessage, SenderKeyMessage, ServerErrorReceipt,
  SyncActionData, SyncActionValue, SyncdMutation, SyncdMutations, SyncdPatch, SyncdRecord,
  SyncdSnapshot, VerifiedNameCertificate, WebMessageInfo, ContextInfo, ExitCode
};
