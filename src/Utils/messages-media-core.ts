...
export const toBuffer = async (stream: Readable | Uint8Array) => {
	if (isBytes<Buffer>(stream)) return stream
	const chunks: Buffer[] = []
...
export const getStream = async (item: WAMediaUpload, opts?: RequestInit & { maxContentLength?: number }) => {
	if (isBytes<Buffer>(item)) {
		return { stream: isReadable(createReadable()) ? toReadable(item) : item, type: 'buffer' } as const
	}
	...
	if (urlStr.startsWith('data:')) {
		...
		return { stream: isReadable(createReadable()) ? toReadable(buffer) : buffer, type: 'buffer' } as const
	}
...
export const encryptMediaRetryRequest = (
	...
	const nodeCompatibleIv = BufferRuntime.from(iv)
	const nodeCompatibleCiphertext = BufferRuntime.from(ciphertext)
	return {
		tag: 'receipt',
		...
		content: [
			{
				...
				attrs: {},
				content: [
					{ tag: 'enc_p', attrs: {}, content: nodeCompatibleCiphertext },
					{ tag: 'enc_iv', attrs: {}, content: nodeCompatibleIv }
				]
			},
			{
				tag: 'rmr',
