 ...
 */
type CallStartResult = string | { readonly callId: string; free?: () => void }

export interface CallAudioBridgeClient {
	acceptCall(callId: string, audioFormat: CallAudioFormat, withVideo?: boolean): Promise<CallStartResult>
	dialCall(peer: string, audioFormat: CallAudioFormat, withVideo?: boolean): Promise<CallStartResult>
	acceptCallPcm(callId: string, withVideo?: boolean): Promise<CallStartResult>
	dialCallPcm(peer: string, withVideo?: boolean): Promise<CallStartResult>
...
const callIdFromStart = (method: string, result: CallStartResult): string => {
	if (typeof result === 'string') {
		assertCallId(method, result)
		return result
	}
	try {
		const id = result?.callId
		assertCallId(method, id)
		return id
	} finally {
		result?.free?.()
	}
...
export const startCallAudioPump = (
	...
	// Keep a pending teardown grace alive until cleanup or the bounded timeout:
	// an awaited socket end must settle even with no other open handles.
	let graceTimer: ReturnType<typeof setTimeout> | undefined
	const teardownGrace = (): Promise<void> =>
		new Promise(resolve => {
			graceTimer = setTimeout(resolve, RELEASE_TEARDOWN_GRACE_MS)
		})
			...
			for (;;) {
				...
				try {
					if (clockMs !== undefined && pulls > 0) {
						...
						if (wait > 0) {
							// An active clock-paced pump needs its next playout tick even
							// when it is the only work left in the process. Cancel the
							// referenced timer promptly if the call stops mid-wait.
							let timer!: ReturnType<typeof setTimeout>
							const paced = new Promise<void>(resolve => {
								timer = setTimeout(resolve, wait)
							})
							try {
								await Promise.race([paced, interruptCurrent])
							} finally {
								clearTimeout(timer)
							}
						}
				...
				if (!skipReleaseWait) {
					await Promise.race([
						...
						(async (): Promise<void> => {
							...
							if (!waitFinished) await teardownGrace()
						})()
					])
				} else {
...
export interface CallAudioMethodHooks {
	...
	/** Set the engine-side relay delegate, even after the client was constructed. */
	setRelayProvider?: (provider: CallRelayTransportProvider) => void
}
...
export const makeCallAudioMethods = (ctx: SocketContext, media: CallMediaRouter, hooks: CallAudioMethodHooks = {}) => {
	...
	return {
		...
		dialCall: async (peerJid: string, audioFormat?: CallAudioFormat, withVideo?: boolean): Promise<string> => {
			...
			const callId = callIdFromStart(
				'dialCall',
				await withAudioClient('dialCall', client => client.dialCall(peerJid, format, withVideo))
			)
		...
		dialCallPcm: async (peerJid: string, withVideo?: boolean): Promise<string> => {
			...
			const callId = callIdFromStart(
				'dialCallPcm',
				await withAudioClient('dialCallPcm', client => client.dialCallPcm(peerJid, withVideo))
			)
		...
		acceptCall: async (callId: string, audioFormat?: CallAudioFormat, withVideo?: boolean): Promise<string> => {
			...
			const format = audioFormat ?? 'mlow'
			const liveId = callIdFromStart(
				'acceptCall',
				await withAudioClient('acceptCall', client => client.acceptCall(callId, format, withVideo))
			)
		...
		acceptCallPcm: async (callId: string, withVideo?: boolean): Promise<string> => {
			...
			const liveId = callIdFromStart(
				'acceptCallPcm',
				await withAudioClient('acceptCallPcm', client => client.acceptCallPcm(callId, withVideo))
			)
		...
		setRelayTransportProvider: (provider: CallRelayTransportProvider): Promise<void> => {
			...
			if (hooks.setRelayProvider) {
				hooks.setRelayProvider(provider)
